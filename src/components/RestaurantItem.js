...
import {ritem} from '../Styles';

const RestaurantItem = ({item , onDetail , onData }) => {
    return (
        <TouchableOpacity 
            style={ritem.touch1}
            onPress={onData}
        >
            <Icon name='cutlery' style={ritem.icon1} size={15} color='#009688' />
            <Text style={ritem.touchText}>{item.name}</Text>
            <TouchableOpacity 
                style={ritem.touch2}
                onPress={onDetail}
            >
                <Icon name='forward' style={ritem.icon2} size={20} color='red' />
            </TouchableOpacity>
